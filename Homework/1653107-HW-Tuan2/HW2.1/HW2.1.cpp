...
void drawLine(HDC hdc, std::vector<LineRect> &arrLRect) {
	for (int i = 0; i < arrLRect.size(); ++i) {
		MoveToEx(hdc, arrLRect[i].x1, arrLRect[i].y1, NULL);
		LineTo(hdc, arrLRect[i].x2, arrLRect[i].y2);
	}
}
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static std::vector<LineRect> arrLRect;
